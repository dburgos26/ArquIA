...
class GraphState(TypedDict):
    ...
    nextNode: Literal["investigator", "diagrams", "creator", "evaluator", "end"]
    imagePath: str
...
supervisorSchema = {
    ...
    "properties": {
        ...
        "nextNode": {
            ...
            "enum": ["investigator", "diagrams", "creator", "evaluator", "end"]
        }
    },
...
def makeSupervisorPrompt(state: GraphState) -> str:
    ...
    These are the possible outputs: ['investigator', 'diagrams', 'creator', 'evaluator', 'end'].
    In cas there is nothing else to do go to end
    """ 
...
def router(state: GraphState):
    if state["nextNode"] == "end":
        return END
    elif state["nextNode"] == "investigator" and state["hasVisitedInvestigator"]:
        return "investigator"
    elif state["nextNode"] == "diagrams" and state["hasVisitedDiagrams"]:
        return "diagrams"
    elif state["nextNode"] == "creator" and state["hasVisitedCreator"]:
        return "creator"
    elif state["nextNode"] == "evaluator" and state["hasVisitedEvaluator"]:
        return "evaluator"
    else:
        return END
...
builder.add_conditional_edges("supervisor", ["investigator", "diagrams", "creator", "evaluator", "end"], router)
...
test = graph.invoke({"messages": [] ,"userQuestion": "What is add 3.0", "localQuestion": "", "hasVisitedInvestigator": False, "hasVisitedDIagrams": False, "hasVisitedCreator": False, "hasVisitedEvaluator": False, "nextNode": "supervisor", "imagePath": ""}, config)
